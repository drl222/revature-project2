...
object Trends {
  ...
  def World_Stats(spark: SparkSession): Unit = {
    ...
    success3.coalesce(1).write.format("csv").option("header", true).mode(SaveMode.Overwrite).save("hdfs://localhost:9000/user/project2/First_Sighting_output.csv")
    success3.show()
    ...
    results1.coalesce(1).write.format("csv").option("header", true).mode(SaveMode.Overwrite).save("hdfs://localhost:9000/user/project2/First_Sighting_Breakdown_output.csv")
    results1.show()
  }
